import React from "react";
import Card from "./Card";

const Team2025 = () => {
  return (
    <div>
      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        TECHNICAL Heads
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card photo="2025/Captain.jpg" name="Himanshu Singh" position="Captain" />
          <Card
            photo="2025/ViceCaptain.jpg"
            name="Prince Kansara"
            position="Vice-Captain"
          />
          <Card
            photo="2025/OperationalManager.jpg"
            name="Abhishek Yadav"
            position="Operational Manager"
          />
          <Card
            photo="2025/ManufacturingHead.jpg"
            name="Jeet Tank"
            position="Manufacturing Head"
          />
          <Card
            photo="2025/ManufacturingHeadElectrical.jpg"
            name="Kaustuv"
            position="Manufacturing Head-Electrical"
          />
        </div>
      </div>

      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        MANAGERIAL Heads
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/CoChairperson.jpg"
            name="Maahir Malde"
            position="Co-Chairperson"
          />
          <Card
            photo="2025/CoChairperson2.jpg"
            name="Aryan Baviskar"
            position="Co-Chairperson"
          />
          <Card
            photo="2025/MarketingHead.jpg"
            name="Burhanuddin Dadawala"
            position="Marketing Head"
          />
          <Card
            photo="2025/MarketingCoHead.jpg"
            name="Sri Vishal"
            position="Marketing Co-Head"
          />
          <Card photo="2025/Treasurer.jpg" name="Devansh" position="Treasurer" />
        </div>
      </div>

      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        CHASSIS
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/ChassisHOD.jpg"
            name="Vatsal Sachaniya"
            position="Head Of The Department"
          />
          <Card
            photo="2025/ChassisMember1.jpg"
            name="Ninad Wadaskar"
            position="Member"
          />
          <Card
            photo="2025/ChassisMember2.jpg"
            name="Rabbit Sinwar"
            position="Member"
          />
          <Card
            photo="2025/ChassisMember3.jpg"
            name="Himanshu Singh"
            position="Member"
          />
          <Card photo="2025/ChassisMember4.jpg" name="Jeet Tank" position="Member" />
        </div>
      </div>

      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        Steering & Suspension
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/SNSHOD.jpg"
            name="Aayushman Singh"
            position="Head Of The Department"
          />
          <Card
            photo="2025/SNSMember1.jpg"
            name="Swastik Choudhary"
            position="Member"
          />
          <Card
            photo="2025/SNSMember2.jpg"
            name="Aryan Baviskar"
            position="Member"
          />
          <Card
            photo="2025/SNSMember3.jpg"
            name="Abhishek Yadav"
            position="Member"
          />
          <Card
            photo="2025/SNSMember4.jpg"
            name="Burhanuddin Dadawala"
            position="Member"
          />
        </div>
      </div>

      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        Powertrain
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/PowertrainHOD.jpg"
            name="Ashok Dudi"
            position="Head Of The Department"
          />
          <Card
            photo="2025/PowertrainMember1.jpg"
            name="Aashlesh Paridhi"
            position="Member"
          />
          <Card
            photo="2025/PowertrainMember2.jpg"
            name="Ajith Shaikh"
            position="Member"
          />
          <Card
            photo="2025/PowertrainMember3.jpg"
            name="Devansh"
            position="Member"
          />
          <Card
            photo="2025/PowertrainMember4.jpg"
            name="Maahir Malde"
            position="Member"
          />
        </div>
      </div>

      <div className="my-5 py-2 uppercase text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        Brakes
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/BrakesHOD.jpg"
            name="Hritik Raj"
            position="Head Of The Department"
          />
          <Card
            photo="2025/BrakesMember1.jpg"
            name="Anurag Parihar"
            position="Member"
          />
          <Card
            photo="2025/BrakesMember2.jpg"
            name="Prateek Verma"
            position="Member"
          />
          <Card
            photo="2025/BrakesMember3.jpg"
            name="Yashaswini Sharma"
            position="Member"
          />
        </div>
      </div>

      <div className="my-5 uppercase py-2 text-center text-2xl font-serif font-bold text-white bg-blue-950 border-2 rounded-3xl">
        Electrical
      </div>
      <div className="flex justify-center">
        <div className="flex flex-wrap justify-center w-full">
          <Card
            photo="2025/ElectricalHOD.jpg"
            name="Kaustuv"
            position="Head Of The Department"
          />
          <Card photo="2025/ElectricalMember1.jpg" name="Akshay" position="Member" />
          <Card
            photo="2025/ElectricalMember2.jpg"
            name="Hrishik"
            position="Member"
          />
          <Card
            photo="2025/ElectricalMember3.jpg"
            name="Preetham"
            position="Member"
          />
          <Card
            photo="2025/ElectricalMember4.jpg"
            name="Rohan Prajapati"
            position="Member"
          />
          <Card photo="2025/ElectricalMember5.jpg" name="Vidhan" position="Member" />
          <Card
            photo="2025/ElectricalMember6.jpg"
            name="Prince Kansara"
            position="Member"
          />
          <Card
            photo="2025/ElectricalMember7.jpg"
            name="Sri Vishal"
            position="Member"
          />
        </div>
      </div>
    </div>
  );
};

export default Team2025;
